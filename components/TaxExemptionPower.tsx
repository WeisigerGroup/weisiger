  ...
  useEffect(() => {
    ...
    const initializeWizard = (token: string) => {
      if (vertex && vertex.Wizard) {
        new vertex.Wizard({
          domNode: document.getElementById('power-btn'),
          wizardPath: 'https://ccwizard.vertexsmb.com/',
          accessToken: token,
          action: "CREATE",
          sellerCodes: ['POWER PRODUCTS'],
        });
...
export const LinkButton: BrandedButtonFunctionType = TaxExemptionPower;
export default TaxExemptionPower;
